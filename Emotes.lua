...
-- LINK MODIFICADO AQUÍ
loadstring(game:HttpGet("https://raw.githubusercontent.com/iivxnu/mis-scripts/refs/heads/main/Emotes.lua"))()
...
endlocal function updateFavoriteIcon(imageLabel, assetId, isFavorite)
...
endlocal function toggleFavorite(emoteId, emoteName)
    local found = false
    local index = 0
...
local function fetchAllAnimations()
    ...
    isLoading = false
endlocal function searchEmotes(searchTerm)
    if isLoading then
        getgenv().Notify({
            Title = '7yd7 | Emote',
...
endlocal function toggleSpeedEmote()
...
-- MODIFICACIÓN PARA QUE SE ABRA CON COMA EN LUGAR DE PUNTO
...
if UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled then
    -- LINK MODIFICADO AQUÍ
    loadstring(game:HttpGet("https://raw.githubusercontent.com/iivxnu/mis-scripts/refs/heads/main/OpenEmoteComma.lua"))()
    getgenv().Notify({
...
if UserInputService.KeyboardEnabled then
    getgenv().Notify({
        Title = '7yd7 | Emote PC',
        Content = '💻 Open menu press button "," (comma)',
        Duration = 10
    })
end
